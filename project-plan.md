...
A successful request (HTTP 200) always returns one of three JSON shapes, the `WorkerResponse` union. The iOS app decodes whichever it receives. Every other status returns the separate error shape described at the end of this section.
...
**Error responses** (every non-200 status; corrected 2026-09-24, Codex review: this section used to say the Worker "always" returns one of the three shapes above):
```json
{ "error": "You've reached your daily limit of 5 questions. Your quota resets at midnight UTC." }
```
Typed as `ErrorResponse` in `types.ts`, and built in one place, `errorResponse()` in `index.ts`. The iOS app switches on the **HTTP status** (step 36's `APIError`), not on this body. `error` is a human-readable message, not a code to match on.

| Status | When |
|---|---|
| 400 | Body isn't JSON, or isn't a valid `messages` array ending with a user turn |
| 401 | Missing or wrong `X-App-Secret` (checked before anything else) |
| 404 | Any path other than `/ask` |
| 405 | `/ask` with a method other than POST (with an `Allow: POST` header) |
| 413 | Body over `MAX_BODY_LENGTH` |
| 429 | Daily question limit reached (8.3) |
| 500 | Unhandled error, e.g. an Anthropic API failure. A Claude **timeout** is not a 500: it returns the 200 `FALLBACK_ANSWER` text envelope (step 46) |
...
**Sanity-check every returned series (Codex review, 2026-09-24).** The parsers only fail when *zero* rows survive, so an upstream change that breaks most rows, or more likely **switches units**, would produce a thin or wrongly scaled chart that looks normal and is labeled with our unit (Section 4). Each tool therefore passes its parsed series through `checkSeries` (`tools/parse.ts`) before returning it. A failure is `tool_parse_failed`, the drift signal, and isn't cached (8.2).
- **Range, on every series:** deliberately wide, so it never rejects real data but catches a unit or scale change: CO₂ 250–700 ppm, CH₄ 1000–3000 ppb, N₂O 250–450 ppb, sea ice 0–20 million km², surface temperature anomaly ±5 °C, ocean heat ±200 ×10²² J, city temperature −90 to 60 °C (Earth's recorded extremes are −89.2 and 56.7 °C).
- **Minimum record count, only on series that always return their full record,** set at ~85% of the count measured live on 2026-09-24. The counts differ a lot by series, so each gets its own floor: GML CO₂ 40/480 (47/570 measured, annual/monthly), CH₄ 36/440 (42/515), **N₂O 20/260 (25/305: its global record starts in 2001)**, and sea ice 40 per month (47–48). Range-requested series (NCEI surface temperature, city) get no count floor, because their count depends on the request and on publication lag.
- **Not yet measured:** NCEI ocean heat. Only world 0–700m has a floor (60, from the 71-row fixture). NCEI's file server returned 503s and timeouts on 2026-09-24 (R14), and the 0–2000m series start decades later, so the other seven files have the range check only. **Follow-up:** measure them when NCEI is healthy, and add floors to `OHC_MIN_POINTS` in `noaaNcei.ts`.
- **Tests (`seriesChecks.spec.ts`)** check both directions. Every real fixture passes its tool's limits. A truncated file, ppm→ppb, km²→thousand km², a ×10 ocean-heat scale and °C→K each fail as `tool_parse_failed`. All five of those fail with the checks disabled.
...
Verification is empirical, not assumed: `claude.ts` logs each round's `usage` fields (kept on permanently, not a debug flag. Codex suggested sampling them, but at our scale they cost nothing: roughly 1–7 lines per request × the ~500–1,000 requests/day that KV writes allow (R4) is at most ~7,000 events a day, about 3% of the Workers Logs free tier's 200,000/day, per Cloudflare's docs 2026-09-24. They're also needed again at the step-40 model-choice gate. If traffic ever grows, set `observability.head_sampling_rate` in `wrangler.jsonc`, with no code change), and the Phase 3 smoke test (step 25) checks that `cache_read_input_tokens` is nonzero on rounds 2+ of a multi-round tool question. Zero reads mean the prefix fell short of the minimum or something is invalidating it (e.g. non-deterministic tool ordering) — investigate, don't ignore. Known accepted edge: a single-round request (refusal or no-tool answer) whose prefix does cache pays the 1.25× write premium with no read — negligible.
